...
uv run uvicorn ikusa.api:app --port 9000 --log-level warning &
API_PID=$!
...
for i in $(seq 1 60); do
  STATUS=$(curl -sf http://localhost:9000/scan/$SCAN_ID | python3 -c "import json,sys; d=json.load(sys.stdin); print(f\"{d['status']}|{d.get('stage','')}|{d.get('message','')}\")")
  echo "[$i] $STATUS"
  case "$STATUS" in
    done\|*) break ;;
...
# Download PDF.
curl -sf -o /tmp/ikusa_api_report.pdf http://localhost:9000/scan/$SCAN_ID/report
